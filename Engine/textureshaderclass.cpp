...
{
#include "Shaders\Compiled shaders\texture.ps.h"
#include "Shaders\Compiled shaders\texture.vs.h"
}
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = device->CreateVertexShader(TextureShader::g_VSMain, sizeof(TextureShader::g_VSMain), NULL, &m_vertexShader);
	if(FAILED(result))
	{
		return false;
	}
	...
	result = device->CreatePixelShader(TextureShader::g_PSMain, sizeof(TextureShader::g_PSMain), NULL, &m_pixelShader);
	if(FAILED(result))
	{
		return false;
	}
