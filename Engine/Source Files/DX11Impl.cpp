...
namespace Engine
{
	...
	DX11Impl::DX11Impl(const DX11Impl& other)
	{
	}
	...
	void DX11Impl::Initialize_Internal(EngineCreateOptions createOptions)
	{
	...
	void DX11Impl::GetAdapterInformation()
	{
	...
	void DX11Impl::CreateDeviceAndSwapChain()
	{
	...
	void DX11Impl::CreateSwapChain_HWND(uint32_t screenWidth, uint32_t screenHeight)
	{
	...
	void DX11Impl::CreateSwapChain_XAML(uint32_t screenWidth, uint32_t screenHeight)
	{
		if (m_swapChain == nullptr)
		{
			//DXGI_SWAP_CHAIN_DESC1 swapChainDesc;
	...
	void DX11Impl::ResizeBuffers(uint32_t newWidth, uint32_t newHeight)
	{
	...
	void DX11Impl::BeginScene(float red, float green, float blue, float alpha)
	{
		float color[4];
	...
	void DX11Impl::UnbindAllRenderTargets() const
	{
	...
	void DX11Impl::UnbindShaderResourceView(uint32_t slot) const
	{
	...
	ID3D11Device* DX11Impl::GetDevice()
	{
		return m_device.Get();
	}


	ID3D11DeviceContext* DX11Impl::GetDeviceContext()
	{
		return m_deviceContext.Get();
	}


	void DX11Impl::GetVideoCardInfo(char* cardName, int& memory)
	{
	...
	Utils::Maths::Vector2 DX11Impl::GetScreenSize() const
	{
	...
	void DX11Impl::SetRenderTarget(RenderTargetBundle::Ptr bundle, Utils::Maths::Vector2 clipOffset /*= {}*/) const
	{
	...
	Texture::Ptr DX11Impl::CopyBackBuffer() const
	{
		return CopyTexture(m_backBufferRT->GetTexture());
	}

	void DX11Impl::BeginRenderEvent(const std::wstring& eventLabel)
	{
