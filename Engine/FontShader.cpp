...
// Include compiled shader byte arrays
namespace FontShaders
{
#include "Shaders\Compiled shaders\font.ps.h"
#include "Shaders\Compiled shaders\font.vs.h"
}
...
bool FontShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, Utils::Maths::Matrix worldMatrix, Utils::Maths::Matrix viewMatrix,
...
bool FontShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
    ...
    result = device->CreateVertexShader(FontShaders::g_VSMain, sizeof(FontShaders::g_VSMain), NULL, &m_vertexShader);
    if(FAILED(result))
    {
        return false;
    }
    ...
    // Create the vertex shader from the buffer.
    result = device->CreatePixelShader(FontShaders::g_PSMain, sizeof(FontShaders::g_PSMain), NULL, &m_pixelShader);
    if(FAILED(result))
    {
        return false;
    }
...
bool FontShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, Utils::Maths::Matrix worldMatrix, Utils::Maths::Matrix viewMatrix,
    Utils::Maths::Matrix projectionMatrix, ID3D11ShaderResourceView* texture, Utils::Maths::Color pixelColor)
{
    HRESULT result;
